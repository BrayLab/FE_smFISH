...
if ~isempty(find(strcmp(varargin, 'ylabel')))
    yLabelText = varargin{find(strcmp(varargin, 'ylabel')) + 1};
end
...
%%CHATGPT MODIFIED CODE TO ENSURE X AXIS LABELS WORK
% Set xticks to match the number of violins
xticks(1:size(Y, 2));

% Ensure xL matches the number of violins
if isempty(xL) == 0
    if length(xL) == size(Y, 2)
        xticklabels(xL); % Assign the correct labels
    else
        error('The number of x-axis labels (xL) must match the number of violins.');
    end
end

% Optionally rotate labels for better visibility
xtickangle(45);
...
% Add axis labels at the end
if ~isempty(yLabelText)
    ylabel(yLabelText, 'FontSize', 15); % Add y-axis label
end

% xlabel(xLabelText, 'FontSize', 12); % Add x-axis label
% ylabel(yLabelText, 'FontSize', 12); % Add y-axis label
