...
% Remove NaNs from each column and compute means
for i = 1:size(AllPropFINAL, 2)
    % Remove NaNs from the i-th column
    columnWithoutNaN = AllPropFINAL(~isnan(AllPropFINAL(:,i)), i);
    
    % Store the mean of the cleaned column
    Mean(i) = mean(columnWithoutNaN);
end
...
Ylabel = 'Proportion of active transcription sites (%)'
...
%saveas(fig7,['ATS' name '.png'])
%print('myfigure', '-dpng')  % Saves the current figure as 'myfigure.png'
print('/Users/gulat/Documents/MyFigures/ATScount.pdf','-fillpage', '-dpdf')
