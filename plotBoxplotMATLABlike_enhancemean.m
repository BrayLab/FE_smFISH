%xfunction[] = plotBoxplot(All,Nicknames,ExpLabels,Jitter,BarW,Title,FontSize,DotSize,CMAP,FaceAlpha,LineWidth,FontSizeTitle,Ylim, Xlabel, Ylabel)
function xfunction = plotBoxplot(All, Nicknames, ExpLabels, Jitter, BarW, Title, FontSize, DotSize, CMAP, FaceAlpha, LineWidth, FontSizeTitle, Ylim, Xlabel, Ylabel)
...
hold on;
groups = unique(CatX(:)); % Unique categories
for i = 1:length(groups)
    idx = (CatX(:) == groups(i)); % Find points in this group
    scatter(CatX(idx)+(rand(sum(idx),1)-0.5).*Jitter, All(idx), DotSize, CMAP(i, :), 'filled', 'MarkerFaceAlpha', 0.5);
end
hold off;
...
%Bar with the median
MedianPlot = plot(Cat,repmat(nanmedian(All),2,1),'-','DisplayName','median','LineWidth', LineWidth * 2);
% 
% for i = 1:size(CMAP, 1)
...
for i = 1:size(CMAP, 1)
    MedianColorMap(i, :) = [CMAP(i, :), min(FaceAlpha + 0.3, 1)]; % Increase transparency slightly
end
set(MedianPlot, {'color'}, num2cell(MedianColorMap, 2));
...
MeanPlot = gscatter(1:length(Nicknames), nanmean(All), 1:length(Nicknames), CMAP, 'x', DotSize, 'LineWidth', 20)
...
xlabel(Xlabel, 'FontSize', FontSize); % Add x-axis label
ylabel(Ylabel, 'FontSize', FontSize); % Add y-axis label
