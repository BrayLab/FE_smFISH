...
Ylabel = 'Cytoplasmic foci count per cell'
...
nexttile
plotBoxplotMATLABlike_v2(AllIntensityHnt, Nicknames, Nicknames, 0.5, 0.2, 'Cytoplasmic Hnt mRNA', 15, 15, CMAP, 0.3, 2,20, [0, 50], Xlabel, Ylabel) %include the first boxplot here
% plotBoxplot(All,Nicknames,ExpLabels,Jitter,BarW,Title,FontSize,DotSize,CMAP,FaceAlpha,LineWidth,FontSizeTitle,Ylim)
nexttile
%include the second boxplot here
plotBoxplotMATLABlike_v2(AllIntensityM7, Nicknames, Nicknames, 0.5, 0.2, 'Cytoplasmic m7 mRNA', 15, 15, CMAPBr, 0.6, 2,20, [0, 20], Xlabel, Ylabel)
% 
% %% Ttest
% 
...
% 'bw', 200) %change this parameter for the appearance of the violins
 ...
 function output = cytoplasmFunc(input1,input2)
    ...
    IntDen = height(CytoplasmIntData);
    TotalIntDen = IntDen
    ATSdata = readtable (input2)
    CellCount = ATSdata{1,2}
    CytoplasmIntensity = (TotalIntDen)/(CellCount/100)
    disp(CytoplasmIntensity)
