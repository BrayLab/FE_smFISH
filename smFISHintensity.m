%script to analyse intensities of ATS
...
for i = 1:size(directories, 2)
    ...
    for k = 1:size(allwrifolders{i}, 2)
      ...
      %loop for wri subfolders
        ...
        HntcsvFilesInFolder = dir([stageiwriPath ,'/*Hntintensities.csv']);
        m7csvFilesInFolder = dir([stageiwriPath ,'/*m7intensities.csv']);
    ...
    for l = 1:size(allBrRNAifolders{i}, 2)
      %loop for all BrRNAi subfolders
        ...
        %obtain all csvs in each wri subfolders
        HntcsvFilesInFolder = dir([stageiBrRNAiPath ,'/*Hntintensities.csv']);
        m7csvFilesInFolder = dir([stageiBrRNAiPath ,'/*m7intensities.csv']);
        % If .csv files are found, store their full paths
        % ~negates expression!!
        if ~isempty(HntcsvFilesInFolder)
            for j = 1:size(HntcsvFilesInFolder, 1)
    ...
    % to trset for s = 1:2
        ...
        if strcmp(masterCellArray{i, 1}, 'stage 6')
            ...
            if strcmp(masterCellArray{i,3} , 'm7')
                %next line added bc my dumbdumb fiji macro didn't work
                %right  
                if strcmp(masterCellArray{i,2} , 'wRi')
                    IntDen = padconcatenation(IntDen, nan, 2)
...
% 
% %%UIP: u need to change the X and Y depending on groups u wanna compare
% % Sample data for two groups: assign X to first matrix, y to second
% X = AllIntensityHnt(:,5);  % Group 1
% Y = AllIntensityHnt(:,6);      % Group 2
% 
% %remove Nans
% X_clean = X(~isnan(X))
% Y_clean = Y(~isnan(Y))
...
% [p, h, stats] = ranksum(X_clean, Y_clean);
% 
% % Display the results
% fprintf('p-value: %.4f\n', p);
% if h == 1
...
Ylabel = 'Intensity of ATS spot (A.U.)'
...
nexttile
plotBoxplotMATLABlike_enhancemean(AllIntensityHnt, Nicknames, Nicknames, 0.5, 0.2, 'Intensities of Active Transcription Sites at Hnt target', 17, 10, CMAP, 0.2, 2,20, [0, 5000], Xlabel, Ylabel)
%plotBoxplotMATLABlike_v2(AllIntensityHnt, Nicknames, Nicknames, 0.5, 0.2, 'Intensities of Active Transcription Sites at Hnt target', 17, 10, CMAP, 0.2, 2,20, [0, 5000], Xlabel, Ylabel)
% plotBoxplotMATLABlike_v2(AllIntensityHnt, Nicknames, Nicknames, 0.5, 0.2, 'No. of nascent Hnt mRNA molecules at Active Transcription Sites', 17, 8, CMAP, 0.2, 2,20, [0, 50], Xlabel, Ylabel) %include the first boxplot here
% plotBoxplot(All,Nicknames,ExpLabels,Jitter,BarW,Title,FontSize,DotSize,CMAP,FaceAlpha,LineWidth,FontSizeTitle,Ylim)
nexttile
%include the second boxplot here
plotBoxplotMATLABlike_enhancemean(AllIntensitym7, Nicknames, Nicknames, 0.5, 0.2, 'Intensities of Active Transcription Sites at m7 target', 18, 10, CMAPBr, 0.4, 2,20, [0, 500], Xlabel, Ylabel)
%plotBoxplotMATLABlike_v2(AllIntensitym7, Nicknames, Nicknames, 0.5, 0.2, 'Intensities of Active Transcription Sites at m7 target', 18, 10, CMAPBr, 0.4, 2,20, [0, 500], Xlabel, Ylabel)
%plotBoxplotMATLABlike_v2(AllIntensitym7, Nicknames, Nicknames, 0.5, 0.2, 'No. of nascent m7 mRNA molecules at Active Transcription Sites', 18, 10, CMAPBr, 0.4, 2,20, [0, 20], Xlabel, Ylabel)

fig = gcf; 

% Set the paper size to landscape (e.g., A4 landscape size)
set(fig, 'PaperOrientation', 'landscape');
set(fig, 'PaperSize', [11, 8.5]);  % Example for A4 landscape size in inches

% Optionally, set the paper position for better fitting
set(fig, 'PaperPosition', [0, 0, 11, 8.5]);  % Adjust as needed

print(fig, 'MyPlot_Landscape', '-dpdf');
